...
export class step {
    ...
    //
    constructor(public id: string, public navigator: navigator.navigator) {
    }
    //
    //Help the user determine the current step s/he is in.
    async execute(i: number, max: number): Promise<navigator.response> {
        //
        ...
        const result: navigator.response = await new Promise((resolve) => {
            //
            ...
            navigator.navigator.next!.onclick = () => {
                ...
                this.done();
                //
                if (i < max) resolve({dir: "next"});
                //
                if (i === max - 1) resolve({dir: "break", ok: true});
            };
            //
            ...
            navigator.navigator.prev!.onclick = () => {
                ...
                if (i > 0) resolve({dir: "prev"});
                else alert("You can not move back beyond this point");
            };
            //
            //Abort the steps.
            navigator.navigator.cancel!.onclick = () => {
                ...
                const ans: boolean = window.confirm('Do you really want to abort the process');
                //
                if (ans === true) resolve({dir: "break", ok: false});
            }
        });
        //
        return result;
